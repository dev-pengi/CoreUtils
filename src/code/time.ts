 ...
 */
const StringToMs = (timeString: string): number => {
    if (!timeString || !timeString.trim().length)
        throw new Error("Time string can't be empty");

    const suffixes: { suffix: string; factor: number }[] = [
        {
            suffix: 's',
            factor: 1000,
        },
    ...
    const timeSuffix = suffixes.find(
        (suffix) =>
            timeString.charAt(timeString.length - 1).toLowerCase() === suffix.suffix
    );

    if (!timeSuffix)
        throw new Error('The suffix must be: s, m, h, d, w, M, y');

    const timeNumber = Number(timeString.slice(0, -1));
 ...
 * @param {string} [separator=' ,'] - Separate between units
 ...
 */
function theNextDayOn(dayOffset: number = 1): number {
